...
{
    public class GuardEffect : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var col = new Color(255, 255, 255) * alphaPercent * 1.5f;
            for (var j = 0; j < 12; j++)
            {
                Vector2 rand = Main.rand.NextVector2Square(-j / 4, j / 4);
                sb.Draw(tex, Projectile.Center - Main.screenPosition + rand, sourceRectangle, Projectile.GetAlpha(col), Projectile.rotation, origin, Projectile.scale * (1f + 0.02f * j), SpriteEffects.None, 0);
            }
            return false;
        }
    }
